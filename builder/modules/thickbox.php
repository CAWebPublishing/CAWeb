...
class ET_Builder_Module_Thickbox extends ET_Builder_CAWeb_Module{
	...
	function get_fields() {
		$fields = array(
			'thickbox_layout' => array(
				...
				'affects' => array('button_link', 'content_new'),
				'toggle_slug' => 'style',
			),			
			'iframe_width' => array(
	...
	function shortcode_callback($atts, $content = null, $function_name) {
		...
		if("inline" == $thickbox_layout){
			$link = sprintf('<a class="thickbox" href="#TB_inline?width=%1$d&height=%2$d&inlineId=%3$s">%4$s</a>', $iframe_width, $iframe_height, $thickbox_id, $button_text );
			
			$output = sprintf('<div id="%1$s" style="display:none">%2$s</div>%3$s', $thickbox_id, $this->shortcode_content, $link );
			
		}elseif("external" == $thickbox_layout ){
			$output = sprintf('<a class="thickbox" href="%1$s?TB_iframe=true&width=%2$d&height=%3$d">%4$s</a>',  $button_link,$iframe_width, $iframe_height, $button_text );

		}

		add_thickbox();
		return sprintf('<div%1$s>%2$s</div>', $class , $output);

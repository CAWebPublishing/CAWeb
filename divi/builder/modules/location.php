...
class ET_Builder_CA_Location extends ET_Builder_CAWeb_Module {
	...
	function render( $unprocessed_props, $content = null, $render_slug ) {
		...
		if ( 'contact' === $location_layout ) {
			...
			$output = sprintf(
				'<div%1$s%2$s><div class="thumbnail">%3$s</div><div class="contact"><div class="title">%4$s</div><div class="address">%5$s</div></div><div class="summary">%6$s%7$s</div></div>',
				$this->module_id(),
				$class,
				$featured_image,
				$name,
				( ! empty( $address ) ? sprintf( ' <span class="ca-gov-icon-road-pin"></span>%1$s', caweb_get_google_map_place_link( $address ) ) : '' ),
				( ! empty( $desc ) ? sprintf( '<div class="title">Description</div><div class="description">%1$s</div>', $desc ) : '' ),
				$display_button
			);
		}

...
class CAWeb_Module_Post_Handler extends ET_Builder_CAWeb_Module {
		...
		$news_fields = array(
			...
			'news_publish_date_custom_format' => array(
				...
				'description'     => et_get_safe_localization(
					sprintf(
						__( 'For formatting help visit <a href="%1$s" target="_blank" title="Formatting Date and Time">Formatting Date and Time</a>', 'et_builder' ),
    ...
    function renderEventDetail($postID){
        ...
        return sprintf('%1$s<div class="description">%2$s</div>%3$s%4$s%5$s%6$s', 
            $this->caweb_get_the_post_thumbnail(null, 'thumbnail', array('class'=>'img-left pr-3')), $this->content, $presenter, $organizer, $reg, $this->renderFooter($postID));
    ...
    function renderExamDetail($postID){
        ...
        return sprintf('<div class="header">%1$s%2$s</div>%3$s%4$s', 
            $this->caweb_get_the_post_thumbnail(null, 'medium', array('class' => 'd-block mb-3')), $exam_info, $this->content, $this->renderFooter($postID));

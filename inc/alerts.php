		...
		if ( $cookie !== false && 'on' === $status &&
			...
			<div  id="caweb-alert-<?php print esc_attr( $a ); ?>" style="background-color: <?php print esc_attr( $color ); ?>" class="alert alert-dismissible mb-0 border-top border-dark py-2 alert-<?php print esc_attr( $a ); ?>">
				<div class="container d-flex flex-row align-items-center">
					...
					<a href="<?php print esc_url( $url ); ?>" target="<?php print empty( $target ) ? '_self' : '_blank'; ?>" class="alert-link btn btn-outline-dark btn-sm ms-auto"><?php print esc_html( $text ); ?></a>
					<?php endif; ?>

					<button data-alert="<?php print esc_attr( $a ); ?>" class="btn border-0 bg-transparent ca-gov-icon-close-mark close caweb-alert-close position-relative p-1<?php print esc_attr( empty( $button ) || empty( $url ) ? ' ms-auto' : '' ); ?>" data-bs-dismiss="alert" aria-label="Close Alert <?php print esc_attr( $a ); ?>"><span class="sr-only">x</span></button>
				</div>
			</div>
			<?php
		}

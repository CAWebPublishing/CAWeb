	...
	class CAWeb_Nav_Menu extends Walker_Nav_Menu {
		...
		public function caweb_nav_menu( $nav_menu, $args ) {
			...
			if ( 'header-menu' === $theme_location && ! empty( $args->menu ) ) {
				...
				$nav_menu = sprintf(
					'<footer id="footer" class="global-footer hidden-print"><div class="container footer-menu"><div class="group">%1$s</div></div><!-- Copyright Statement --><div class="copyright"><div class="container"><p class="d-inline"><span aria-hidden=true> %copy </span>; %2$s State of California</p>%3$s</div></div></footer>',
					$nav_menu,
					gmdate( 'Y' ),
					$powered_by
				);
		...
		public function createSubNavMenu( $child_links, $args ) {
			...
			foreach ( $child_links as $i => $item ) {
				...
				if ( isset( $item_meta['_caweb_menu_icon'] ) && ! empty( $item_meta['_caweb_menu_icon'][0] ) ) {
					$icon = sprintf( '<span class="ca-gov-icon-%1$s%2$s" aria-hidden="true"></span>', 
				...
				if( 'flexmega' === $args->style ){
					...
					if( 'unit3' === $unit_size ){
						...
						if( 'left' === $nav_media_alignment ){
							...
							$body = sprintf( '<div class="media-body"><div class="title text-left">%1$s</div>%2$s</div>', 
								$item->title,
								$desc							
							);
						}

						$media = sprintf('%1$s<img class="%2$s" src="%3$s" alt="%4$s">%5$s',
							$media_wrap,
							$media_class,
							$nav_media_image,
							$nav_media_alt_text,
					...
					/* Create Link */
					$link = sprintf('<a href="%1$s" class="second-level-link"%2$s>%3$s%4$s</a>', 
							$item->url,
							( ! empty( $item->target ) ? sprintf( ' target="%1$s"', $item->target ) : '' ),
							$media,
							$body
					...
					if( $new_row ){
						...
						$link = sprintf( '</div><%1$s class="%2$s%3$s">%4$s', 
							$element,
							$class ,
					...
					if ( 'unit3' !== $unit_size ) {
						/* Create Link */
						$link = sprintf('<a href="%1$s" class="second-level-link"%2$s>%3$s%4$s%5$s</a>', 
							$item->url,
							( ! empty( $item->target ) ? sprintf( ' target="%1$s"', $item->target ) : '' ),
							$icon,
							$item->title,

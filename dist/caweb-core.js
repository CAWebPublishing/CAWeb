...
jQuery(document).ready(function($) {
	...
	gtag('config', 'UA-3419582-2', {cookie_flags:'secure;samesite=lax;domain='+document.domain});

	// CAWeb Multisite analytics
	if(undefined !== args.caweb_multi_ga){
		gtag('config', args.caweb_multi_ga, {cookie_flags:'secure;samesite=lax;domain='+document.domain});
	}
	
	// Agency UA ID
	if( undefined !== args.ca_google_analytic_id){
		gtag('config', args.ca_google_analytic_id, {cookie_flags:'secure;samesite=lax;domain='+document.domain});
	}
...
jQuery(document).ready(function($) {
	...
	if( undefined !== args.ca_google_analytic4_id){
		gtag('config', args.ca_google_analytic4_id, {cookie_flags:'secure;samesite=lax;domain='+document.domain}); // individual agency - either from your own google account, or contact eServices to have one set up for you
	}

	gtag('config', 'G-69TD0KNT0F', {cookie_flags:'secure;samesite=lax;domain='+document.domain}); // statewide analytics - do not remove or change

	if( undefined !== args.caweb_multi_ga4 ){
		gtag('config', args.caweb_multi_ga4, {cookie_flags:'secure;samesite=lax;domain='+document.domain}); // CAWeb multisite analytics - do not remove or change
	}

			...
			if ( ! $caweb_item->menu_item_parent ) {
				...
				?>
					...
					>
						...
						<?php
						if ( ! empty( $caweb_child_items ) ) {
							$caweb_first_row_border = get_post_meta( $caweb_child_items[0]->ID, '_caweb_menu_flexmega_border', true );
							?>
								<div class="sub-nav">
									<div class="second-level-nav flex<?php print esc_attr( $caweb_first_row_border ? ' with-border' : '' ); ?>">
									<?php
									foreach ( $caweb_child_items as $caweb_child_item ) {
										...
										$caweb_new_row              = isset( $caweb_child_item_meta['_caweb_menu_flexmega_row'][0] ) ? $caweb_child_item_meta['_caweb_menu_flexmega_row'][0] : '';
										$caweb_border               = isset( $caweb_child_item_meta['_caweb_menu_flexmega_border'][0] ) ? $caweb_child_item_meta['_caweb_menu_flexmega_border'][0] : '';
										...
										$caweb_nav_media_alignment = isset( $caweb_child_item_meta['_caweb_menu_media_image_alignment'][0] ) ? $caweb_child_item_meta['_caweb_menu_media_image_alignment'][0] : 'left';

										if ( $caweb_new_row ) {
											// close the open second-level-nav and open another for the new row.
											?>
												</div>
												<div class="second-level-nav flex<?php print esc_attr( $caweb_border ? ' with-border' : '' ); ?>">
											<?php
										}
										?>
												<a 
												...
												<?php if ( ! empty( $caweb_child_item->xfn ) ) : ?>
														...
														<?php endif; ?>
													>
												<?php if ( 'unit1' === $caweb_child_item_unit_size ) : ?>
														...
														<?php elseif ( 'unit2' === $caweb_child_item_unit_size ) : ?>

															<?php if ( ! empty( $caweb_child_item_icon ) ) : ?>
															<span class="ca-gov-icon-<?php print esc_attr( $caweb_child_item_icon ); ?>" aria-hidden="true"></span>
															<?php endif; ?>

															<p class="h3 sub-nav-link"><?php print esc_html( $caweb_child_item->title ); ?></p>

															<?php if ( ! empty( $caweb_child_item->description ) ) : ?>
																<div class="link-description"><?php print esc_html( $caweb_child_item->description ); ?></div>
															<?php endif; ?>

														<?php else : ?>

															<?php if ( 'left' === $caweb_nav_media_alignment ) : ?>

																<?php if ( ! empty( $caweb_nav_media_image ) ) : ?>
																<div class="media-left">
																	<img 
																		class="media-object width-80 height-80" 
																...
																<div class="media-body">
																	<div class="title text-left">
																		<p class="h3 sub-nav-link"><?php print esc_html( $caweb_child_item->title ); ?></p>
																		<?php if ( ! empty( $caweb_child_item->description ) ) : ?>
																			<div class="link-description text-left">
																				<?php print esc_html( $caweb_child_item->description ); ?>
																			</div>
																		<?php endif; ?>
																	</div>
																</div>
															<?php else : ?>
																...
																<?php if ( ! empty( $caweb_nav_media_image ) ) : ?>
																	<img 
																		class="image-icon rounded-50x m-b-md" 
																		src="<?php print esc_url( $caweb_nav_media_image ); ?>" 
																		alt="<?php print esc_attr( $caweb_nav_media_alt_text ); ?>" />
																<?php endif; ?>
																<p class="h3 sub-nav-link"><?php print esc_html( $caweb_child_item->title ); ?></p>

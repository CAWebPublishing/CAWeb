			...
			<div class="social-media-links">
				<div class="header-cagov-logo">
					<a href="https://www.ca.gov/" title="CA.gov website">
						...
						<img style="height: 31px;" src="<?php print get_stylesheet_directory_uri(); ?>/images/system/logo.svg" class="pos-rel" alt="CA.gov website" aria-hidden="true" />
					</a>
				</div>
				...
				foreach ( $social_share as $opt ) {
					...
					if ( get_option( $opt . '_header' ) && ( $share_email || '' !== get_option( $opt ) ) ) {
						...
						printf(
							'<a class="utility-social-%1$s ca-gov-icon-%1$s" href="%2$s" title="Share via %3$s" %4$s><span class="sr-only">%3$s</span></a>',
							$share,
							( $share_email ? $mailto : get_option( $opt ) ),
							ucwords( $share ),
							( get_option( $opt . '_new_window' ) ? 'target="_blank"' : '' )
						);
					}
				}
				?>
